...
new_state_dict = OrderedDict()#the model was trained on the parallel gpu,so there is a problem here.we need to rename the dict keys
...
test_gen = MultiModalDatasetTTA(test_files,config.test_data,config.test_vis,mode="test")
...
with torch.no_grad():
	for i,(input,visit,filepath) in tqdm(enumerate(test_loader)):
		...
		visit = torch.from_numpy(np.array([visit.numpy(),visit.numpy(),
		visit.numpy(),visit.numpy(),visit.numpy(),visit.numpy()])).view(-1,visit.shape[1],visit.shape[2],visit.shape[3])
	
		image_var = input.to(device)
		visit=visit.to(device)
		
		y_pred1 = model1(image_var,visit)
		y_pred1 = y_pred1.view(batch_size*n_crops, -1).mean(0,keepdim=True)
		y_pred2 = model2(image_var,visit)
		y_pred2 = y_pred2.view(batch_size*n_crops, -1).mean(0,keepdim=True)
