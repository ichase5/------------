...
from utils import *
from multimodal import MultiModalDataset,MultiModalDatasetTTA,MultiModalNet,CosineAnnealingLR
...
torch.cuda.manual_seed_all(1996)
...
# 2. evaluate function
def evaluate(val_loader,model,criterion,epoch,train_metrics,best_results,start):#TTA
	...
	with torch.no_grad():
		for i, (images,visit,target) in enumerate(val_loader):
			...
			# visit = torch.from_numpy(np.array([visit.numpy(),visit.numpy(),visit.numpy(),visit.numpy(),visit.numpy(),visit.numpy()])).view(-1,visit.shape[1],visit.shape[2],visit.shape[3])
...
def test(basenet,test_loader,model,folds):
	...
	for i,(input,visit,filepath) in tqdm(enumerate(test_loader)):
		...
		# batch_size, n_crops, c, h, w = input.size()
		...
		with torch.no_grad():
			...
			#y_pred = y_pred.view(batch_size, n_crops, -1).mean(1)
	...
	sample_submission_df.to_csv('./submit/%s_bestloss_submission.csv'%basenet, index=None)
...
def main():
	...
	#4.3 optim & criterion
	optimizer = optim.SGD(model.parameters(),lr = config.lr,momentum=0.9,weight_decay=1e-4)#weight_decay为L2 regularization
	...
	train_data_list,val_data_list = train_test_split(all_files, test_size=0.1, random_state = 1996)
	...
	# test(basenet,test_loader,model,fold)
