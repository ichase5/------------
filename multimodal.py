...
class MultiModalDataset(Dataset):
	...
	def augumentor(self,image):
		augment_img = iaa.Sequential([
			...
			], random_order=True) #不同batch的操作顺序不同

		image_aug = augment_img.augment_image(image)
		return image_aug


class MultiModalDatasetTTA(Dataset):
	def __init__(self,images_df, base_path,vis_path,mode):
		if not isinstance(base_path, pathlib.Path):
			base_path = pathlib.Path(base_path)
		if not isinstance(vis_path, pathlib.Path):
			vis_path = pathlib.Path(vis_path)
	...
	def __getitem__(self,index):
		...
		images = [
					T.ToTensor()(T.functional.hflip(X)), 
					T.ToTensor()(T.functional.vflip(X)), 
					T.ToTensor()(X),
					T.ToTensor()(T.functional.rotate(X,90)),
					T.ToTensor()(T.functional.rotate(X,180)),
					T.ToTensor()(T.functional.rotate(X,270)),
				]
		images = torch.from_numpy(np.array([image.numpy() for image in images]))
		
		return images.float(),visit.float(),y
...
class MultiModalNet(nn.Module):
	def __init__(self, backbone1, backbone2, drop, pretrained=False):  #网络原因，下载不了，只能从头训练了
		super().__init__()
		if pretrained:
		...
		self.visit_model=DPN26()#无预训练

		self.img_encoder = list(img_model.children())[:-2]
		#去掉的最后两层是:
		#AvgPool2d(kernel_size=(7, 7), stride=(1, 1), padding=0)
		#Linear(in_features=2048, out_features=1000, bias=True)
		self.img_encoder.append(nn.AdaptiveAvgPool2d(1)) #变为1*1的平面尺寸
